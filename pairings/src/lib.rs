...
use engines::{bls12_381_engine, bls12_461_engine, bls24_479_engine, bls24_559_engine, bls48_575_engine};
                           ...
                           {  f = f.sqr();                                
                              for i in 0..q_list.len() 
                                    {  let l = double_jacobian_for_miller(&mut _qlist[i],&_plist[i].x,&_plist[i].y);
                                       f = f.sparse_multiply(&[ l[idx.0].content(),l[idx.1].content(),l[idx.2].content()]);
                                    }   
