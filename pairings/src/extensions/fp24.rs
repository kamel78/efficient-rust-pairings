...
impl <const PARAMSIZE:usize,const N:usize> ExtElement<PARAMSIZE,24,N> for Fp24Element<PARAMSIZE,N>{
    ...
    fn content_interface(&self) -> &[super::super::fields::prime_fields::FieldElement<N>;24]
        {   &self.content   }
    ...
    fn sqr(&self) -> Self {
        ...
        let t1 = b.sqr();
        let t2 = c.sqr();
        let h0 = b.addto(&c).sqr().substract(&t1).substract(&t2).mulby_u_p_1().addto(&t0);
        let h1 = t2.mulby_u_p_1().addto(&a.addto(&b).sqr().substract(&t0).substract(&t1));
    ...
    fn multiply(&self, rhs:&Self) -> Self {          
        ...
        let h0 = b0.addto(&c0).multiply(&b1.addto(&c1)).substract(&t1).substract(&t2).mulby_u_p_1().addto(&t0);        
        let h1 = t2.mulby_u_p_1().addto(&a0.addto(&b0).multiply(&a1.addto(&b1)).substract(&t0).substract(&t1));
...
impl <const PARAMSIZE:usize,const N:usize> Fp24Element <PARAMSIZE,N>{
    ...
    pub fn invert(&self) -> Self {
        ...
        let b = get_slice_fp8(&self, 1);
        let c = get_slice_fp8(&self, 2);
        let t0 = a.sqr().substract(&b.multiply(&c).mulby_u_p_1());
        let t1 = c.sqr().mulby_u_p_1().substract(&a.multiply(&b));
        let t2 =b.sqr().substract(&a.multiply(&c));                                    
        let t3 = c.multiply(&t1).addto(&b.multiply(&t2)).mulby_u_p_1().addto(&a.multiply(&t0)).invert();
        let mut result =[FieldElement{  mont_limbs:self.content[0].fieldparams.zero,
                                                               fieldparams:self.content[0].fieldparams};24];
    ...
    pub fn unisqr(&self) -> Self {
       ...
       let h00= t1.mulby_u_p_1().addto(&t0).mulbyu8(3u8).substract(&a0.addto(&a0));
       let h01= t2.substract(&t0).substract(&t1).mulbyu8(3u8).addto(&a1.addto(&a1));
       let t0 = a4.sqr();
       let t1 = a5.sqr();
       let t2 = a4.addto(&a5).sqr();
       let h10= t2.substract(&t0).substract(&t1).mulby_u_p_1().mulbyu8(3u8).addto(&a2.addto(&a2));
       let h11= t1.mulby_u_p_1().addto(&t0).mulbyu8(3u8).substract(&a3.addto(&a3));
       let t0 = a2.sqr();
       let t1 = a3.sqr();
       let t2 = a2.addto(&a3).sqr();
       let h20= t1.mulby_u_p_1().addto(&t0).mulbyu8(3u8).substract(&a4.addto(&a4));
    ...
    pub fn sparse_multiply(&self, rhs:&[&[FieldElement<N>];3]) -> Self {   
            //  sparse multiplication of an Fp24 element with a sparse element on Fp8 only
            ...
            let res1 = t2.mulby_u_p_1().addto(&t0);
            let res2 = a0.addto(&b0).multiply(&y0.addto(&y1)).substract(&t0).substract(&t1);
            let res3 = c0.multiply(&y0).addto(&t1);           
            let mut result =[FieldElement{ mont_limbs:self.content[0].fieldparams.zero,
                                                                    fieldparams:self.content[0].fieldparams};24];
    ...
    pub fn sparse_multiply_for48(&self, rhs:&[&[FieldElement<N>];3], mode :u8) -> Self {         
        match  mode { 1 => { //  Sparse multiplication of an Fp24 element with a sparse element on two consecutive Fp8 only
                            ...
                            let res1 = t1.mulby_u_p_1().addto(&t0);
                      ...
                      2 => { //  Sparse multiplication of an Fp24 element with a sparse element on two consecutive Fp8 only
                            ...
                            let res1 = c0.multiply(&y).mulby_u_p_1();
