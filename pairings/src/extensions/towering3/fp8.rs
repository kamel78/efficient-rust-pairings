...
impl <const PARAMSIZE:usize,const N:usize> Fp8Element <PARAMSIZE,N>{
    ...
    pub fn invert(&self) -> Self {
        ...
        let b= Fp4Element{   content : [self.content[4],self.content[5],self.content[6],self.content[7]], 
                                               constants:self.constants};
        let t = a.sqr().substract(&b.sqr().mulby_v().negate()).invert();
        let t0 = a.multiply(&t);
    ...
    pub fn is_qr(&self) -> bool{        
            ...
            let b = Fp4Element { content : [self.content[4],self.content[5],self.content[6],self.content[7]], 
                                                constants: self.constants};
            a.sqr().substract(&b.sqr().mulby_v().negate()).is_qr()            
            }
    ...
    pub fn sqrt(&self) -> Option<Self>{
        ...
        let t0= a.sqr();
        let t1= b.sqr();       
        let rootdelta = t0.substract(&t1.mulby_v().negate()).sqrt();                                 
    ...
    pub fn mulby_w(&self) -> Self {
            Fp8Element {content : [self.content[6].substract(&self.content[7]), self.content[6].addto(&self.content[7]),
                                   self.content[4].negate(), self.content[5].negate(), self.content[0], self.content[1], self.content[2], self.content[3]],
                        constants : self.constants}

        }

    pub fn mulby_u(&self) -> Self {
            Fp8Element {content : [self.content[1].negate(), self.content[0],self.content[3].negate(), self.content[2], 
                                   self.content[5].negate(), self.content[4], self.content[7].negate(), self.content[6]],
                        constants : self.constants}
    ...
    pub fn sparse_multiply(&self, rhs:&[FieldElement<N>]) -> Self {                
                ...
                let r0 = t0.addto(&t1.mul_by_u_p_1().negate());
                let _x = a0.addto(&a1);
                ...
                let r2 = t0.addto(&t1.mul_by_u_p_1().negate());            
                let _x = a2.addto(&a3);
                let x = [_x.content[0].addto(&_x.content[1]),_x.content[0].multiply(&y0py2),_x.content[1].multiply(&y1py3)];
