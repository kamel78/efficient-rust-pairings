...
pub mod fp6 {
    ...
    impl <const PARAMSIZE:usize,const N:usize> ExtElement<PARAMSIZE,6,N> for Fp6Element<PARAMSIZE,N>{
        ...
        fn sqr(&self) -> Self {
            ...
            let t1 = b.sqr();
            let t2 = c.sqr();
            let x0 = c.addto(&b).sqr().substract(&t1).substract(&t2).mul_by_u().addto(&t0);
            let x1 = a.addto(&b).sqr().substract(&t0).substract(&t1).addto(&t2.mul_by_u());
        ...
        fn multiply(&self, rhs : &Self) -> Self {  
            ...
            let x0 = b0.addto(&c0).multiply(&b1.addto(&c1)).substract(&t1).substract(&t2).mul_by_u().addto(&t0);
            let x1 = a0.addto(&b0).multiply(&a1.addto(&b1)).substract(&t0).substract(&t1).addto(&t2.mul_by_u());
    ...
    impl <const PARAMSIZE:usize,const N:usize> Fp6Element <PARAMSIZE,N>{
        pub fn invert(&self) -> Self {
            ...
            let b = Fp2Element{content :[self.content[2],self.content[3]]};
            let c = Fp2Element{content :[self.content[4],self.content[5]]}; 
            let t0 = a.sqr().substract(&b.multiply(&c).mul_by_u());
            let t1 = c.sqr().mul_by_u().substract(&a.multiply(&b));
            let t2 = b.sqr().substract(&a.multiply(&c));                
            let t = c.multiply(&t1).addto(&b.multiply(&t2)).mul_by_u().addto(&a.multiply(&t0)).invert();
...
impl <const PARAMSIZE:usize,const N:usize> ExtElement<PARAMSIZE,12,N> for Fp12Element<PARAMSIZE,N>{
    ...
    fn content_interface(&self) -> &[super::super::fields::prime_fields::FieldElement<N>;12]
        {   &self.content   }
...
impl <const PARAMSIZE:usize,const N:usize> Fp12Element <PARAMSIZE,N>{
    ...
    pub fn unisqr(&self) -> Self { 
        ...
        let z1 = a2.sqr();
        let z2 = a3.addto(&a2);
        let t0 = x0.addto(&x1.mul_by_u()).mulbyu8(3u8).substract(&a0.addto(&a0));
        let t1 = z0.addto(&z1.mul_by_u()).mulbyu8(3u8).substract(&a1.addto(&a1));
        let t2 = y0.addto(&y1.mul_by_u()).mulbyu8(3u8).substract(&a2.addto(&a2));
        let t3 = y2.sqr().substract(&y0).substract(&y1).mul_by_u().mulbyu8(3u8).addto(&a3.addto(&a3));
