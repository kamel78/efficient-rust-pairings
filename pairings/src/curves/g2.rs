...
impl <const PRAMASIZE:usize,const R:usize,const N:usize,const MAX_COEFS_COUNT :usize> G2Element<PRAMASIZE,R,N,MAX_COEFS_COUNT>
        {   
            ...
            {   match self.point.x {    ExtFieldG2Element::Fp2(_) => 2,
                                        ExtFieldG2Element::Fp4(_) => 4,
                                        ExtFieldG2Element::Fp8(_) => 8,
                                    } 
            }
            ...
            {
                match self.point.x {    ExtFieldG2Element::Fp2(_) => gls4_multiply_bls12(&self, scalar),
                                        ExtFieldG2Element::Fp4(_) => gls8_multiply_bls24(&self, scalar),
                                        ExtFieldG2Element::Fp8(_) => gls16_multiply_bls48(&self, scalar),
                                   }    
            }            
            ...
            {
                match self.point.x {    ExtFieldG2Element::Fp2(_) => clean_cofactor_bls12(&self),
                                        ExtFieldG2Element::Fp4(_) => clean_cofactor_bls24(&self),
                                        ExtFieldG2Element::Fp8(_) => clean_cofactor_bls48(&self),
                                   }               
            }
            pub fn phi(&self) -> Self
            {   
                match self.point.x {    ExtFieldG2Element::Fp2(_) => phi_bls12(&self),
                                        ExtFieldG2Element::Fp4(_) => phi_bls24(&self,1),
                                        ExtFieldG2Element::Fp8(_) => phi_bls48(&self,1)
                                   }       
            }
...
impl <const PRAMASIZE:usize,const R:usize,const N:usize,const MAX_COEFS_COUNT : usize> G2Field<PRAMASIZE,R,N,MAX_COEFS_COUNT> 
        {   
            ...
            {   match self.base_field {    ExtG2Field::Fp2(_) => 2,
                                           ExtG2Field::Fp4(_) => 4,
                                           ExtG2Field::Fp8(_) => 8
                                      } 
            }

            fn map_to_curve(&self,seed :&ExtFieldG2Element<N, PRAMASIZE>) -> G2Element<PRAMASIZE,R,N,MAX_COEFS_COUNT>
            { 
